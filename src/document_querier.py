...
class DocumentQuerier:
    ...
    def query_document(self, siren: str, year: int, save_path: str, s3: bool = True):
        """
        ...
        """
        availability, document_id = self.check_document_availability(siren, year)
        if not availability:
            raise KeyError(f"No document found for {siren} in {year}.")
        self.download_from_id(document_id, save_path, s3)
        return

    def check_document_availability(self, siren: str, year: int) -> Tuple[bool, str]:
        """
        Query a document using the INPI API. Save at save_path.
    ...
    def download_from_id(self, identifier: str, save_path: str, s3: bool = True):
        """
