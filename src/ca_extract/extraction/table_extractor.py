...
class TableExtractor:
    ...
    def extract(
        ...
        plt.savefig(os.path.join(get_root_path(), "output/raw_column_masks.png"))
        plt.clf()

        segmented_tables = self.process_tables(self.segment_table_mask(raw_table_mask))
        ...
        for table in segmented_tables:
            ...
            if segmented_columns:
                ...
                plt.savefig(os.path.join(get_root_path(), "output/column_masks.png"))
                plt.clf()
    ...
    def process_columns(self, segmented_columns: np.array) -> Dict[int, np.array]:
        """
