...
class ColumnAssembler:
    ...
    def assemble(self, column_list: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        mean_top_diffs = [
            (df.top.unique()[1:] - df.top.unique()[:-1]).mean() for df in column_list
        ]
        ...
        diffs = [
            a - b
            for a, b in zip(all_cols.top.tolist(), ([0] + all_cols.top.tolist()[:-1]))
        ]
        all_cols["diffs"] = diffs
        all_cols["new_line"] = np.where(all_cols.diffs > threshold, 1, 0)
        all_cols["line_id"] = all_cols.new_line.cumsum()
        formatted_cols = [
            all_cols.loc[all_cols.df_index == i] for i in range(len(column_list))
        ]
        formatted_cols = [
