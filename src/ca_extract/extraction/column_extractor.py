...
class ColumnExtractor:
    ...
    def extract_columns(self, column_masks: Dict, image: Image) -> List[pd.DataFrame]:
        """
        ...
        if self.readjust:
            readjusted_column_masks = self.readjust_column_masks(column_masks, image)
        else:
        ...
        plt.savefig(os.path.join(get_root_path(), "output/readj_column_masks.png"))
        plt.clf()
    ...
    def extract_column(self, column_mask: np.array, image: Image) -> pd.DataFrame:
        """
    ...
    def readjust_column_masks(self, column_masks: Dict, image: Image) -> Dict:
        ...
        for colname, column_mask in column_masks.items():
            ...
            scaled_hull = binary_dilation(hull, square(self.readjust_dilation_diameter))
    ...
    def column_to_dataframe(self, column_mask: np.array, image: Image) -> pd.DataFrame:
        """
        ...
        crop = crop + white
        ocr = image_to_string(Image.fromarray(crop.astype(np.uint8)), lang="fra")
        return pd.DataFrame(
    ...
    def column_to_dataframe_with_loc_legacy(
        ...
        hull = convex_hull_image(column_mask)
        scaled_hull = binary_dilation(hull, square(self.final_dilation_diameter))
    ...
    def column_to_dataframe_with_loc(
        ...
        filtered_df_indexed["line"] = (filtered_df["block_num"] - 1) * filtered_df[
            "line_num"
        ] + filtered_df["line_num"]
        filtered_df_indexed = filtered_df_indexed.groupby("line").agg(
            {"top": "min", "text": lambda x: " ".join(x)}
        )
        filtered_df_indexed = filtered_df_indexed.rename(columns={"text": "col"})[
            ["col", "top"]
        ].sort_values("top")
