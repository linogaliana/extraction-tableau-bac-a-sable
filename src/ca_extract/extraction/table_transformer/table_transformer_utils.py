"""
Utils for post-processing TableTransformer outputs.
"""
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    recognition model into a *consistent* set of table structures
    (rows, columns, spanning cells, headers). This entails resolving
    conflicts/overlaps, and ensuring the boxes meet certain alignment
...
def cells_to_csv(cells):
    ...
    df = pd.DataFrame(
        table_array[max_header_row + 1 :, :],
        index=None,
        columns=flattened_header,
    )
...
def apply_class_thresholds(bboxes, labels, scores, class_names, class_thresholds):
    """
    Filter out bounding boxes whose confidence is below the
    confidence threshold for its associated class label.
    """
...
def objects_to_table_structures(
    ...
    Process the bounding boxes produced by the table structure
    recognition model into a *consistent* set of table structures
    (rows, columns, supercells, headers). This entails resolving
    conflicts/overlaps, and ensuring the boxes meet certain alignment
    conditions (for example: rows should all have the same width, etc.).
    """
    ...
    columns = [
        obj for obj in objects_in_table if class_names[obj["label"]] == "table column"
    ]
    rows = [obj for obj in objects_in_table if class_names[obj["label"]] == "table row"]
    headers = [
        obj
        for obj in objects_in_table
...
def nms_by_containment(container_objects, package_objects, overlap_threshold=0.5):
    """
    Non-maxima suppression (NMS) of objects based on shared containment of other
    objects.
    """
    ...
    final_objects = [
        obj for idx, obj in enumerate(container_objects) if not suppression[idx]
    ]
...
def slot_into_containers(
    ...
    """
    Slot a collection of objects into the container they occupy most (the
    container which holds the largest fraction of the object).
    """
    best_match_scores = []
...
def remove_objects_without_content(page_spans, objects):
    """
    Remove any objects (these can be rows, columns, supercells, etc.) that
    don't have any text associated with them.
    """
    for obj in objects[:]:
...
def extract_text_from_spans(
    spans, join_with_space=True, remove_integer_superscripts=True
):
    """
...
def refine_table_structures(table_bbox, table_structures, page_spans, class_thresholds):
    ...
    # Process supercells
    supercells = [
        elem for elem in table_structures["supercells"] if not elem["subheader"]
    ]
...
def nms(
    ...
    match_threshold: the cutoff for determining that overlap requires suppression of
    one object keep_higher: if True, keep the object with the higher metric;
...
def align_supercells(supercells, rows, columns):
    ...
    for supercell in supercells:
        ...
        if (
            ...
            if (
                "span" in supercell
                and supercell["header"]
                and len(supercell["column_numbers"]) > 1
            ):
...
def header_supercell_tree(supercells):
    """
    Make sure no supercell in the header is below more than one supercell
    in any row above it. The cells in the header form a tree, but a supercell
    with more than one supercell in a row above it means that some cell has
    more than one parent, which is not allowed. Eliminate
...
def table_structure_to_cells(table_structures, table_spans, table_bbox):
    ...
    for column_num, column in enumerate(columns):
        for row_num, row in enumerate(rows):
            ...
            for supercell in supercells:
                ...
                if (
                    supercell_rect.intersect(cell_rect).get_area()
                    / cell_rect.get_area()
                ) > 0.5:
                    cell["subcell"] = True
                    break
    ...
    for supercell in supercells:
        ...
        for subcell in subcells:
            ...
            subcell_rect_area = subcell_rect.get_area()
            if (
                subcell_rect.intersect(supercell_rect).get_area() / subcell_rect_area
            ) > 0.5:
                if cell_rect is None:
                    cell_rect = Rect(list(subcell["bbox"]))
                else:
...
def refine_table_structure(table_structure, class_thresholds):
    ...
    column_headers = apply_threshold(
    ...
    projected_row_headers = apply_threshold(
