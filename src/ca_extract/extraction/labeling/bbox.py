    ...
    def extract_coordinates(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if not self.column:
                ...
                cv2.rectangle(self.img2, self.last_coord, (x, y), self.color, 2)
            else:
            ...
            if self.drawing:
                ...
                if a != x & b != y:
                    ...
                    cv2.rectangle(self.img, self.last_coord, (a, b), self.color, 2)
            ...
            print(
                "top left: {}, bottom right: {}".format(temp_coord[-2], temp_coord[-1])
            )

            cv2.rectangle(self.img2, temp_coord[-2], temp_coord[-1], self.color, 2)
            self.img2 = self.img.copy()
...
if __name__ == "__main__":
    ...
    for file in img_files:
        ...
        if (
            ...
            img_base = cv2.resize(img_base, (int(screen_height / ratio), screen_height))
            bounding_box_widget = BoundingBoxWidget(img_base)
            cv2.namedWindow("Tables")
            cv2.setMouseCallback("Tables", bounding_box_widget.extract_coordinates)
            stop_loop = True
            while stop_loop:
