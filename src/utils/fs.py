...
def extract_content_from_file(
    ...
    return extract_document_content(doc, resolution, parallel, maxthreads)
...
def extract_ocr_page(page: fitz.Page, resolution: int = 200):
    ...
    ocr = image_to_data(image, lang="fra", config="--psm 1", output_type="data.frame")
...
def clean_page_content(page_content: str) -> str:
    ...
    words_no_numbers = [stemmer.stem(word) for word in words if word not in stopwords]
    # Remove accents
    clean_content = " ".join([unidecode.unidecode(word) for word in words_no_numbers])
...
def train_random_forest(params: Dict, X_train: List[List[int]], y_train: List[int]):
    """
...
def load_extra_labeled_data():
    ...
    flat_corpus_with_number = [clean_page_content(page) for page in flat_corpus]
    flat_corpus = [remove_number(page) for page in flat_corpus_with_number]
    valid_labels = list(df.tableau_f_et_p)

    num_rates = []
    num_rates = [get_numeric_char_rate(content) for content in flat_corpus_with_number]
